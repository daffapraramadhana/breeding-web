...
import { analyzeBatchPnl, fetchPaginated, ApiError } from "@/lib/api";
import {
  AiInsightResponse,
  BatchPnlAnalysis,
...
export function BatchPnlAnalysisTab() {
  const [filters, setFilters] = useState<BatchAnalysisRequest>({});
  const [result, setResult] = useState<AiInsightResponse<BatchPnlAnalysis> | null>(null);
  ...
  async function handleAnalyze(forceRefresh = false) {
    ...
    try {
      ...
      const data = await analyzeBatchPnl(params);
      setResult(data);
    } catch (err) {
    ...
    <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setFilters({
                    ...filters,
                    status: v === "ALL" ? undefined : (v as "ACTIVE" | "CLOSED"),
                  })
                }
              >
                <SelectTrigger>
      ...
      {!isLoading && insight && (
        <div className="space-y-6">
          ...
          {/* Analisis per Batch */}
          {insight.analisis_batch.length > 0 && (
            <InsightCard title="Analisis per Batch">
              <div className="rounded-md border">
                <Table>
                  ...
                  <TableBody>
                    {insight.analisis_batch.map((batch) => (
                      <TableRow key={batch.batch_number}>
                        <TableCell className="font-medium">
                          {batch.batch_number}
                        </TableCell>
                        <TableCell>
                          <PenilaianBadge penilaian={batch.penilaian} />
                        </TableCell>

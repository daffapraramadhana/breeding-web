...
export default function BonusIpPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (editing) {
        ...
        toast.success("Bonus IP updated successfully");
      } else {
        await fetchApi("/bonus-ip", {
          method: "POST",
          body: JSON.stringify(body),
        });
        toast.success("Bonus IP created successfully");
      }
      ...
      toast.error(
        editing
          ? "Failed to update bonus IP"
          : "Failed to create bonus IP"
      );
    } finally {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast.success("Bonus IP deleted successfully");
      setDeleteDialogOpen(false);
      setDeleting(null);
      refetch();
    } catch {
      toast.error("Failed to delete bonus IP");
    }
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Bonus IP"
        description="Manage bonus IP configurations"
        actions={
      ...
      <DataTable
        ...
        searchPlaceholder="Search bonus IP..."
        page={page}
        totalPages={meta?.totalPages || 1}
        onPageChange={setPage}
        total={meta?.total}
        emptyTitle="No bonus IP found"
        emptyDescription="Get started by creating your first bonus IP."
        emptyAction={
          <Button onClick={handleCreate}>
            <Plus className="mr-2 h-4 w-4" />
            New Bonus IP
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              {editing ? "Edit Bonus IP" : "New Bonus IP"}
            </DialogTitle>
            <DialogDescription>
              {editing
                ? "Update the bonus IP details below."
                : "Fill in the details to create a new bonus IP."}
            </DialogDescription>
          </DialogHeader>
